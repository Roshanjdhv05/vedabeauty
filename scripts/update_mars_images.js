...
async function updateImages() {
  try {
    console.log("--- UPDATING MARS PRODUCT IMAGES ---");

    // Skip brand creation due to RLS. Use products directly.

    // 2. Fetch all products where brand_name is 'Mars' or 'MARS'
    const { data: products, error: pError } = await supabase
      .from('products')
      .select('id, name, brand_name')
      .or(`brand_name.ilike.Mars`);
    ...
    for (const product of products) {
      ...
      // If no exact match, try partial match (like in the frontend)
      if (!imagePaths) {

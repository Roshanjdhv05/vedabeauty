...
let sql = `-- INSERT INSIGHT PRODUCTS (If you want to run manually)\n`;
sql += `-- Note: These are already in the database, but this SQL file can be used to insert them if needed.\n\n`;
...
for (const item of RAW_PRODUCT_LIST) {
  const sellingPrice = Math.round(item.mrp_price * 0.75);
  const safeName = item.name.replace(/'/g, "''");
  const imageUrl = `/insight/${safeName}.jpg`;

  sql += `
  INSERT INTO products (name, brand_id, category, has_variants, mrp_price, selling_price, price, original_price, discount, image_url)
  VALUES ('${safeName}', insight_brand_id, '${item.category}', false, ${item.mrp_price}, ${sellingPrice}, ${sellingPrice}, ${item.mrp_price}, 25, '${imageUrl}')
...
import fs from 'fs';
fs.writeFileSync('scripts/insert_insight_products.sql', sql);

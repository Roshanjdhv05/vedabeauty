...
Edge of Desire Lip Liner-Holder: Rs. 790/-
...
High Coverage Foundation: Rs. 379/-
...
async function updateMarsPrices() {
  const lines = marsPriceData.split('\n');
  const priceMap = [];
  ...
  lines.forEach(line => {
    if (line.includes(': Rs.')) {
      ...
      const priceStr = parts[1].split('/')[0].trim();
      const price = parseFloat(priceStr);
      if (!isNaN(price)) {
        priceMap.push({ name, mrp: price });
      }
    }
  });

  console.log('Parsed ' + priceMap.length + ' products with prices.');
  ...
  for (const item of priceMap) {
    ...
    if (results.length > 0) {
      ...
      const mrp = item.mrp;
      let selling = Math.round(mrp * 0.75);
      if (selling > 10) selling = Math.floor(selling / 10) * 10 + 9;
